...
void COneIPv6Info::fill_arp_req_buf(uint8_t *p, uint16_t port_id, COneIPInfo *sip) {
    ...
    CTestPktGen::create_neighbor_solicitation(p, ((COneIPv6Info *)sip)->get_ipv6(), reinterpret_cast<uint8_t*>(m_ip), src_mac, m_vlan);
}
...
void COneIPv6Info::fill_grat_arp_buf(uint8_t *p) {
    ...
    CTestPktGen::create_unsolicited_neighbor_advertisement(p, reinterpret_cast<uint8_t*>(m_ip), src_mac, m_vlan);
}
