...
CJsonData_err CAstfDB::verify_data(uint16_t max_threads) {
    ...
    for (int i = 0; i < ip_gen_list.size(); i++) {
        ...
        uint32_t num_ips = ipv4v6_addr::range_size(ip_end, ip_start);
...
void CAstfDB::update_server_info(CTcpServerInfo* server_info) {
    ...
    auto ip_start = assoc["ip_start"] ? ipv4v6_addr::from_str(assoc["ip_start"].asString().c_str()): ipv4v6_addr::ipv4_le(0);
    auto ip_end = assoc["ip_end"] ? ipv4v6_addr::from_str(assoc["ip_end"].asString().c_str()): ipv4v6_addr::ipv4_le(UINT32_MAX);
