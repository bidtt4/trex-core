...
void CUdpFlow::Create(CTcpPerThreadCtx *ctx, uint32_t profile_id,
    ...
    if (client){
        INC_UDP_STAT(m_ctx, m_profile_id, m_tg_id, udps_connects);
    }else{
        INC_UDP_STAT(m_ctx, m_profile_id, m_tg_id, udps_accepts);
    }
}

void CUdpFlow::Delete(){
    INC_UDP_STAT(m_ctx, m_profile_id, m_tg_id, udps_closed);
    disconnect();
}
...
rte_mbuf_t   * CUdpFlow::alloc_and_build(CMbufBuffer *      buf){
    ...
    if ( (hlen+buf->m_t_bytes)>=MAX_PKT_SIZE){
        INC_UDP_STAT(m_ctx, m_profile_id, m_tg_id, udps_pkt_toobig);
        /* drop the packet */
        return(0);
    }
    ...
    if (m==0) {
        INC_UDP_STAT(m_ctx, m_profile_id, m_tg_id, udps_nombuf);
        /* drop the packet */
        return(0);
    }
...
void CUdpFlow::send_pkt(CMbufBuffer *      buf){
    ...
    INC_UDP_STAT(m_ctx, m_profile_id, m_tg_id, udps_sndpkt);
    INC_UDP_STAT_CNT(m_ctx, m_profile_id, m_tg_id, udps_sndbyte,buf->m_t_bytes);
...
void CUdpFlow::on_tick(){
    if (m_keepalive==0) {
        ...
        INC_UDP_STAT(m_ctx, m_profile_id, m_tg_id, udps_keepdrops);
        disconnect();
    }
}
...
void CUdpFlow::on_rx_packet(struct rte_mbuf *m,
    ...
    INC_UDP_STAT(m_ctx, m_profile_id, m_tg_id, udps_rcvpkt);
    INC_UDP_STAT_CNT(m_ctx, m_profile_id, m_tg_id, udps_rcvbyte,total_l7_len);
    m_keepalive=0;
